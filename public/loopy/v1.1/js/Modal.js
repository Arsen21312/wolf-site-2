...
function Modal(loopy){
	...
	(function(){
		...
		label.innerHTML = "????? ???? ??? ?????????? ???????? <span style='text-decoration:underline; cursor:pointer' onclick='publish(\"modal\",[\"examples\"])'>???????!</span>";
	...
	(function(){
		...
		label.innerHTML = "(??? ??????? URL, ??????? ????? ???????????? ??????????? ?????? ????? <a target='_blank' href='https://bitly.com/'>bit.ly</a>)";
		page.dom.appendChild(label);
