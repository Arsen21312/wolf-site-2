  ...
  <section class="decisions-page">
    ...
    <TextType
      ...
      :text="[
        ...
        'Без споров, обид и лишних разговоров.'
      ]"
      :typing-speed="75"
      :pause-duration="1500"
      :show-cursor="true"
    ...
    <div class="grid two">
      <template v-for="tool in tools" :key="tool.title">
        <NuxtLink v-if="tool.slug" :to="tool.slug" class="card decision-card link-card">
          <div class="tag">{{ tool.tag }}</div>
          <h3>{{ tool.title }}</h3>
          <p>{{ tool.description }}</p>
          <div class="chip-row">
            <div class="chip">{{ tool.mood }}</div>
            <div class="chip">{{ tool.speed }}</div>
          </div>
        </NuxtLink>

        <article v-else class="card decision-card">
          <div class="tag">{{ tool.tag }}</div>
          <h3>{{ tool.title }}</h3>
          <p>{{ tool.description }}</p>
          <div class="chip-row">
...
const tools = [
  ...
  },
  {
    ...
    tag: 'Колесо выбора',
    description: 'Крути колесо, чтобы выбрать еду, решение или победителя без споров.',
    mood: 'Азартно',
    speed: '5 секунд',
    slug: '/decisions/wheel-of-fortune'
  },
  {
    title: 'Генератор победителя',
    tag: 'Розыгрыш',
    description: 'Выбери одного или нескольких победителей, настрой фильтры и скачай протокол розыгрыша.',
    mood: 'Прозрачно',
    speed: '1 минута',
    slug: '/decisions/giveaway-winner'
  },
  {
    title: 'Разделить на команды',
    tag: 'Командный баланс',
    description: 'Разбей список людей на команды случайно или с учетом силы участников.',
    mood: 'Честно',
    speed: '30 секунд',
    slug: '/decisions/team-generator'
  },
  {
    title: 'Конвертер чисел',
    tag: 'Сумма прописью',
    description:
      'Бесплатный конвертер чисел в сумму прописью для документов. Поддержка НДС 12% и 20%, правильные склонения.',
  ...
  {
    title: 'Случайная дата',
    tag: 'Диапазон дат',
    description: 'Выбери диапазон и получи случайную дату: будни, выходные, исключения, несколько дат и время.',
    mood: 'Планы и челленджи',
    speed: '5 секунд',
    slug: '/decisions/random-date'
  },
  {
    title: 'Таймер Помидора',
    tag: 'Фокус и отдых',
    description: 'Помодоро таймер с настройками, звуком, уведомлениями и сохранением прогресса.',
    mood: 'Продуктивность',
    speed: 'Запуск за секунду',
    slug: '/decisions/pomodoro'
  },
  {
